...
def findpercents():
    ...
    for j in range(len(L6)): # if L1 then index out of range so take as L6 becoz L6 will be same as L1
        if (L6[j]=='0'):
    ...
    try:
        global percent
        def percent():
            p=((count1+count2)/len(L1))*100
            print("totalcb%=",p)
            p1=(count1/len(L1))*100
            print("malecb%=",p1)
            p2=(count2/len(L1))*100
            print("femalecb%=",p2)
        percent()
...
def  finaloutput():
    global L1,L2,L3,list1,list2,L9,L10,L11,L8,L7,newpop,L6,L66
    print("_______________________________________________________________final output_______________________________________________")
