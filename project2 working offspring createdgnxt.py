...
def findpercents():
    ...
    for j in range(len(L1)): # if L1 then index out of range so take as L6 becoz L6 will be same as L1
        if (L6[j]=='0'):
            ...
            if(L1[j]=='0'):
                count1=count1+1
                print(j,"th person is colorblind(male)")    
            elif(L1[j]=='1'):
                count2=count2+1
                print(j,"th person is colorblind(female)")
    try:
        global percent,p,p1,p2
        def percent():
            global p,p1,p2
            p=((count1+count2)/len(L6))*100
            print("TOTAL % WITH COLORBLINDNESS=",p)
            p1=(count1/len(L6))*100
            print("TOTAL MALE % WITH COLORBLINDNESS=",p1)
            p2=(count2/len(L6))*100
            print("TOTAL FEMALE % WITH COLORBLINDNESS=",p2)
...
def  finaloutput():
    ...
    print("******************************************FINAL OUTPUT FOR GENERATION:",k,"***********************************************************")
...
def main():
    print("__________________________________________________GENETICS ALGORITHM(To detect color blindness for nth generation)__________________________________________")
    ...
    def gen():
        ...
        for k in range(n2):
            print("______________________________________________this is generation:",k,"___________________________________________")
            offspring()
            update()
